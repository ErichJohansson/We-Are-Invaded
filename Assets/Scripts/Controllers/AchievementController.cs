...
public class AchievementController : MonoBehaviour
{
    ...
    public void CheckEligibleAchievements()
    {
        ...
        if(gc.DistanceTraveled >= 1000 && gc.DistanceTraveled < 1337.42f)
        {
            UnlockAchievement(GPGSIds.achievement_sunday_driver);
        }
        if (gc.DistanceTraveled >= 1337.42f && gc.DistanceTraveled < 5000)
        {
            UnlockAchievement(GPGSIds.achievement_1337_d21v32);
        }
        if (gc.DistanceTraveled >= 5000)
        {
        ...
        if (gc.ModifiersCollected >= 25 && gc.ModifiersCollected < 50)
        {
            UnlockAchievement(GPGSIds.achievement_i_love_powerups);
        }
        if (gc.ModifiersCollected >= 50)
        {
